#!/bin/bash
# release.sh — signed + notarized release pipeline for ChePDFMCP (PsychQuant/che-pdf-mcp).
#
...
grep -qE "static let serverVersion[[:space:]]*=[[:space:]]*\"$VERSION_RE\"" "$BUILD_TREE/Sources/ChePDFMCP/Server.swift" \
...
# .build/apple/Products/Release this script assumed (macdoc#163 / che-word-mcp#195).
